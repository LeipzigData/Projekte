...
// 11.02.2015 HGG: lang parameter removed for getLanguage() call
function searchByUri($search)
{
    // search results 
    $searchResults = array();
    ...
    foreach ($graph->allOfType('jsp:Ort') as $strKey) {
        if (preg_match('/'.$search.'/i', $strKey->label())) {
            ...
            if (strlen($latitude) > 0 AND strlen($longitude) > 0) {
                $location["uri"] = 
		  str_replace('http://localhost/jsp/', 
			      'http://leipzig-data.de/Data/Jugendstadtplan/', $strKey);
