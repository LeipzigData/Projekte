 ...
 * User: emanuelott
 * Date: 19.06.13
 * Update: Hans-Gert Gräbe, 2018-10-14
...
function getTextElements() {
    $text_elements=array();
    $text_elements['Details'][0]['de'] =  "[Öffnungszeiten]";
    $text_elements['Details'][0]['en'] =  "[opening hours]";
    $text_elements['Details'][1]['de'] =  "[Eigenschaften]";
    $text_elements['Details'][1]['en'] =  "[characteristics]";
    $text_elements['Details'][2]['de'] =  "[Kontakt]";
    $text_elements['Details'][2]['en'] =  "[contact]";
    $text_elements['Details'][3]['de'] =  "Bitte Ort auswählen.";
    $text_elements['Details'][3]['en'] =  "Please choose a place in the map.";
    $text_elements['Details'][4]['de'] =  "Kein Ort ausgewählt";
    $text_elements['Details'][4]['en'] =  "No place choosen";
    $text_elements['Details'][6]['de'] =  "Informationen anzeigen";
    $text_elements['Details'][6]['en'] =  "show details";
...
function getCategories($quelle,$lang) {
    ...
    foreach ($graph1->resources() as $res) {
        if ($res->hasProperty('jsp:hascategory') == 1)
        {
            ...
            if (!in_array($art, $testarray)) {
                ...
                $out.="<div class='bottomnavi' id='cat".$count."'><a href='#' onclick='disable_marker(\"" . $type . "\",".$count.");showResult(document.getElementById(\"searchfield\").value);'>" . $labelres2 . "</a></div>\n";
            $count++;
            }
            }
    ...
    $out.="<div class='bottomnavi' id='cat".$count."'><a href='#' onclick='disable_marker(\"NA\",".$count.");showResult(document.getElementById(\"searchfield\").value)'>NO Category</a></div>\n";
    $out.="<div class='bottomnavi bottomnavi_selected' id='cat".($count+1)."'><a href='#' onclick='disable_marker(\"all\",".($count+1).");showResult(document.getElementById(\"searchfield\").value);'>Alle Orte</a></div>\n";
    return $out;
}
...
// und Lat und Long extrahiert werden können dann werden diese entweder dem bestehenden Graphen zugeführt
    ...
    if ($do == "turtle") {
        ...
        } else $out.="no Need of changing something inside the rdf";
...
function create_filter_file($quelle)
{

    // Graphen parsen
    $graph = EasyRdf_Graph::newAndLoad($quelle); // Läd den vollen Graphen
    $testarray2 = array();
    $testarray3 = array();
    $testarray = array();
