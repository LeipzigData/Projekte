...
function contentloader(uri, lang) {
    $("#label").load("details.php?lang=" + lang + "&uri=" + uri + " #heading");
...
// send search request to webserver and display response
//======================================================
function searchRequest(searchKey) {
    ...
    $.ajax({
        ...
        success: function(data){
            if(data.length == 0) {
                $('#searchResponse').html('<strong>Sorry!</strong> for your keyword: <strong>"' 
			+ searchKey + '"</strong> we found <strong>0</strong> search results!');
                ...
                $('#searchResponse').html('<strong>Success!</strong> for your keyword: <strong>"' 
			+ searchKey + '"</strong> we found <strong>' + data.length 
			+ '</strong> search results! It took ' + (Date.now() - requestStart) + ' ms!');
...
function getGroupLocations(group) {
    ...
    $.ajax({
        ...
        success: function(data){
            if(data.length == 0) {
                $('#searchResponse').html('<strong>Sorry!</strong> for your selected group: <strong>"' 
			+ group + '"</strong> we found <strong>0</strong> search results!');
                $("#searchResponse").removeClass('alert-success');
                $("#searchResponse").addClass('alert-warning');
                $("#searchResponse").slideDown('slow').delay(4000).slideUp('slow');
            } else {
                $('#searchResponse').html('<strong>Success!</strong> for your selected group: <strong>"' 
			+ group + '"</strong> we found <strong>' + data.length 
			+ '</strong> search results! It took ' + (Date.now() - requestStart) + ' ms!');
                $("#searchResponse").removeClass('alert-warning hide');
                $("#searchResponse").addClass('alert-success');
                $("#searchResponse").slideDown('slow').delay(4000).slideUp('slow');
...
function addLocation(jsp_uri, label, lat, lng, lang) {
	...
	.bindPopup('<b>' + label + '</b><br/><a href="#" onclick="contentloader(\'' + jsp_uri 
		   + '\',\'' + lang + '\');">Informationen anzeigen</a>.');
...
function setFocusToMarker(uri) {
    // center map at the requested location 
