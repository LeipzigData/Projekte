        ...
        if ($res->hasProperty('jsp:hascategory') == 1) {
	      ...
	      if (!in_array($art, $testarray)) { 
		    ...
		    // echo "$type -- $labelres2 <br/>"; 
                    $menuCategories.= '
                    <li role="presentation" class=""><a onclick=\'getGroupLocations("'
		      . $labelres2 .'");\' aria-controls="home'.$count.'" data-toggle="tab" id="home'
		      .$count.'-tab" role="tab" href="#home'.$count
		      .'" aria-expanded="false">'.$labelres2.'</a></li>';
                    $categoriesContent.= '
                    <div aria-labelledby="home'.$count.'-tab" id="home'.$count
...
// echo getCategories("de","alle","keine"); // for test only
